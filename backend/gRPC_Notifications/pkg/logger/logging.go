...
func (l *Logger) shouldLog(requiredLevel uint8) bool {
	...
	if config.LoggerLevel < requiredLevel {
		return false
	}
...
func InitLoggers() {
	...
	settingsFile, err := os.Open(config.AppRoot + "pkg/logger/settings.json")
	if err != nil {
	...
	err = os.MkdirAll(config.AppRoot+"logs", 0777)
	if err != nil {
		panic(err)
	}
	...
	for _, key := range settings[loggersKey].([]interface{}) {
		...
		for _, handler := range handlers {
			...
			case "file":
				file, err := os.OpenFile(config.AppRoot+fmt.Sprintf("logs/%s", handler["filename"].(string)), os.O_APPEND|os.O_CREATE|os.O_RDWR, 0666)
				if err != nil {
					panic(err)
				}
